...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
	...
	// 装载BeanDefinition的map，key是beanName，在注册BeanDefinition的时候放入值
	...
	// 根据beanName从缓存中获取BeanDefinition
	@Override
		...
		if (existingDefinition != null) {
			...
			if (hasBeanCreationStarted()) {
				...
				synchronized (this.beanDefinitionMap) {
					// 把bean放到map中
					this.beanDefinitionMap.put(beanName, beanDefinition);
					// 替换beanDefinitionNames名称的集合
					// 因为ArrayList不支持并发，每次都new了一个新的ArrayList覆盖旧的
					// 使用拷贝的方式，防止添加途中别的地方出现并发操作
					List<String> updatedDefinitions = new ArrayList<>(this.beanDefinitionNames.size() + 1);
					updatedDefinitions.addAll(this.beanDefinitionNames);
					updatedDefinitions.add(beanName);
					this.beanDefinitionNames = updatedDefinitions;
					// manualSingletonNames 同上
	...
	public Object doResolveDependency(DependencyDescriptor descriptor, @Nullable String beanName,
		...
		try {
			...
			// 没有匹配项，但是有设置了required=true报错，否则返回null
