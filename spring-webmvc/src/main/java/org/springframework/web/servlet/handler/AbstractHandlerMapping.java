...
public abstract class AbstractHandlerMapping extends WebApplicationObjectSupport
	 ...
	 */
	/**
	 * 目前是空实现，可供子类扩展，在集合interceptors加入自定义的拦截器
	 */
	 ...
	 */
	/**
	 * 查找spring容器中所有类型是MappedInterceptor的bean，加入到当前拦截器集合中
	 ...
	 */
	/**
	 * 根据传入的拦截器类型，转换为HandlerInterceptor类型或WebRequestInterceptor类型
	 */
	...
	public final HandlerExecutionChain getHandler(HttpServletRequest request) throws Exception {
		...
		// 如果handler是String，直接找Spring容器中有没有name=此handler的bean，找到就用它处理，
	 ...
	 */
	/**
	 * 根据handler获取执行链HandlerExecutionChain，handler如果是HandlerExecutionChain对象会合并到一起
	 */
	protected HandlerExecutionChain getHandlerExecutionChain(Object handler, HttpServletRequest request) {
		...
		// 把当前类包含的拦截器加入到HandlerExecutionChain中，并把HandlerExecutionChain返回出去
		for (HandlerInterceptor interceptor : this.adaptedInterceptors) {
			// MappedInterceptor拦截器需要判断是否在配置的URI范围内
			if (interceptor instanceof MappedInterceptor) {
