...
public abstract class AbstractUrlHandlerMapping extends AbstractHandlerMapping implements MatchableHandlerMapping {
	...
	protected Object getHandlerInternal(HttpServletRequest request) throws Exception {
		...
		if (handler == null) {
			...
			// 如果请求的是/返回rootHandler(在初始化时已经设置了)
			if ("/".equals(lookupPath)) {
				rawHandler = getRootHandler();
			}
			// 不是/，则获取defaultHandler，注册时/*的handler是defaultHandler
			if (rawHandler == null) {
	...
	protected Object lookupHandler(String urlPath, HttpServletRequest request) throws Exception {
		...
		if (handler != null) {
			// Bean name or resolved handler?
			// 获取到bean对象，如果非懒加载（lazyInitHandlers=false，默认配置）不会进此判断，因为初始化时handler已经是bean对象
		...
		// Pattern match?
		// url不完全匹配，遍历所有的缓存url，判断是否满足通配符匹配
		List<String> matchingPatterns = new ArrayList<>();
		for (String registeredPattern : this.handlerMap.keySet()) {
			if (getPathMatcher().match(registeredPattern, urlPath)) {
				matchingPatterns.add(registeredPattern);
			}
			// 没有匹配，判断是否忽略尾部的/，默认不忽略
		...
		// 如果通配符匹配（matchingPatterns有元素）才会有以下逻辑
		// 找出最匹配的url
	...
	protected void registerHandler(String urlPath, Object handler) throws BeansException, IllegalStateException {
		...
		if (mappedHandler != null) {
			...
			// 如果url='/*'设置为默认的handler，如果此类获取不到handler，在父类中取默认的
			...
			// 否则放入handlerMap中，缓存url对应的handler
			else {
