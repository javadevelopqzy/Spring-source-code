...
public final class ModelFactory {
	...
	/**
	 * 包含所有需要执行的方法，配置了@SessionAttributes的方法
	 */
	 ...
	 */
	/**
	 * 构造函数，把配置了@ModelAttribute的handlerMethods保存到this.modelMethods中<br>
	 * 给dataBinderFactory和sessionAttributesHandler赋值
	 */
	...
	public void initModel(NativeWebRequest request, ModelAndViewContainer container, HandlerMethod handlerMethod)
		...
		// sessionAttributes中配置的session属性map
		// 如：@sessionAttributes({"name", "key"})，则map={name=session.getAttribute("name"),key=session.getAttribute("key")}
		Map<String, ?> sessionAttributes = this.sessionAttributesHandler.retrieveAttributes(request);
		// 合并配置了@SessionAttributes类的session属性到mavContainer中，
		container.mergeAttributes(sessionAttributes);
		// 调用所有配置了ModelAttribute注解的方法
	 ...
	 */
	/**
	 * 调用所有配置了@ModelAttribute注解的方法
	 */
	private void invokeModelAttributeMethods(NativeWebRequest request, ModelAndViewContainer container)
			throws Exception {

		while (!this.modelMethods.isEmpty()) {
			// 获取下一个@ModelAttribute注解的方法
			// 先找容器属性名不完全包含参数名的方法，否则取第一个
	...
	/**
	 * 获取下一个方法来执行，先找容器属性名不完全包含参数名的方法，否则取第一个
	 */
	private ModelMethod getNextModelMethod(ModelAndViewContainer container) {
		// 遍历所有需要执行的方法
		for (ModelMethod modelMethod : this.modelMethods) {
			// 容器属性名完全包含方法参数名，返回此方法
	 ...
	 */
	/**
	 * 获取方法中配置了@ModelAttribute注解的参数名称，如果ModelAttribute设置了name属性，则取name
	 */
	...
	private static class ModelMethod {
		...
		/**
		 * 配置了@ModelAttribute的方法
		 */
		private final InvocableHandlerMethod handlerMethod;

		/**
		 * 方法的中配置了@ModelAttribute参数名称，有value属性就取value，否则取参数名称
		 */
		...
		/**
		 * 判断mavContainer容器中的属性是否有不包含方法参数名
		 */
